...
* `SyncSession::OnlyForTesting::handle_error()` now takes a `realm_legacy::sync:SessionErrorInfo` instead of a `realm_legacy::SyncError` to reflect internal refactoring ([PR #6433](https://github.com/realm/realm-core/pull/6433)).
...
### Breaking changes
* Make `realm_legacy::util::Optional<T>` be an alias for `std::optional<T>` and `realm_legacy::none` an alias for `std::nullopt`. ([PR #5667](https://github.com/realm/realm-core/pull/5667)) For now the existing name and some of the helpers are sticking around so this shouldn't affect most consumers, however:
  ...
  * `std::optional` cannot be directly streamed (`<<`) to an `ostream`. A helper `realm_legacy::util::stream_possible_optional` is provided that can be used instead. For convenience, it will work with both optional and non-optional types.
...
### Breaking changes
* `realm_legacy::Realm::Config` has been renamed to `realm_legacy::RealmConfig`. ([#5436](https://github.com/realm/realm-core/pull/5436))
...
* Several typedefs in `realm/object-store/sync/sync_session.hpp` were renamed ([#4924](https://github.com/realm/realm-core/pull/4924)):
  * `realm_legacy::SyncSession::SyncSessionStateCallback` -> `realm_legacy::SyncSession::StateChangeCallback`
  * `realm_legacy::SyncSession::ConnectionStateCallback` -> `realm_legacy::SyncSession::ConnectionStateChangeCallback`
  * `realm_legacy::SyncSessionTransactCallback` -> `realm_legacy::SyncSession::TransactionCallback`
  * `realm_legacy::SyncProgressNotifierCallback` -> `realm_legacy::SyncSession::ProgressNotifierCallback`
  * `realm_legacy::SyncSession::NotifierType` -> `realm_legacy::SyncSession::ProgressDirection`
* `realm_legacy::SyncClientConfig::logger_factory` was changed to a `std::function` that returns logger instances. The abstract class `SyncLoggerFactory` was removed. ([#4926](https://github.com/realm/realm-core/pull/4926))
...
* Fixed a race between calling realm_legacy::delete_files and concurent opening of the realm file.([#4768](https://github.com/realm/realm-core/pull/4768))
...
### Fixed
* You may get assertion "n != realm_legacy::npos" when integrating changesets from the server. ([#4180](https://github.com/realm/realm-core/pull/4180), since v10.0.0)
...
### Internals
* 'using namespace realm_legacy::util' removed from header. This means that this namespace is no longer used automatically.
...
* A use-after-free would occur if a Realm was compacted, opened on multiple threads prior to the first write, then written to while reads were happening on other threads. This could result in a variety of crashes, often inside realm_legacy::util::EncryptedFileMapping::read_barrier. (Since v6.0.0, [Cocoa #6652](https://github.com/realm/realm-cocoa/issues/6652), [Cocoa #6628](https://github.com/realm/realm-cocoa/issues/6628), [Cocoa #6655](https://github.com/realm/realm-cocoa/issues/6555)).
...
### Fixed
* When shutting down the server you could sometimes experience a crash with "realm_legacy::util::Mutex::lock_failed"
  in the stacktrace.
...
### Fixed
* A bug was fixed in `realm_legacy::util::DirScanner` that could cause it to sometimes
...
* A specialised exception realm_legacy::OutOfDiskSpace is thrown instead of a generic
...
* Implemented `realm_legacy::util::File::is_dir`, `realm_legacy::util::File::resolve`,
  and `realm_legacy::util::DirScanner` on Windows.
...
* Add more overloads with realm_legacy::null - PR [#2669](https://github.com/realm/realm-core/pull/2669)
...
* Fixed a crash that occurred if you tried to override a binary with a size close
  to the limit.
  PR [#2416](https://github.com/realm/realm-core/pull/2416).
* `seekpos()` and `seekoff()` in `realm_legacy::util::MemoryInputStreambuf` now behave
...
* Removed `realm_legacy::util::network` library.
...
* Search indexes no longer support strings with lengths greater than
  ...
  (through the Table interface), then a `realm_legacy::LogicError` will be thrown with type
...
with nullable = true. You can then use realm_legacy::null() in place of any
...
* `Group::get_table()` and Group::get_table_name() now throw
  `realm_legacy::InvalidArgument` if the specified table index is out of range.
...
. Class `File` and related exceptions such as `File::AccessError` moved to namespace `realm_legacy::util`.
...
----------
+ 6 realm_legacy::is_a<MyTable>(const Table&) added.
+ 6 realm_legacy::unchecked_cast<MyTable>(TableRef) added.
+ 6 realm_legacy::checked_cast<MyTable>(TableRef) added.
