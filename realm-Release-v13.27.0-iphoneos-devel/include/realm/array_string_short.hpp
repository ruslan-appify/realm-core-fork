...
xxx0 xx01 x002 0003 0004 (strings "xxx",. "xx", "x", "", realm_legacy::null())

So each string is 0 terminated, and the last byte in a block tells how many 0s are present, except
for a realm_legacy::null() which has the byte set to m_width (4). The byte is used to compute the length of a string
in various functions.

New: If m_witdh = 0, then all elements are realm_legacy::null(). So to add an empty string we must expand m_width
...
inline StringData ArrayStringShort::get(size_t ndx) const noexcept
{
    ...
    if (m_width == 0)
        return m_nullable ? realm_legacy::null() : StringData("");
...
{
    add(m_nullable ? realm_legacy::null() : StringData("")); // Throws
}
...
inline StringData ArrayStringShort::get(const char* header, size_t ndx, bool nullable) noexcept
{
    ...
    if (width == 0)
        return nullable ? realm_legacy::null() : StringData("");
