...
template <>
struct hash<realm_legacy::ObjKey> {
    size_t operator()(realm_legacy::ObjKey key) const
    {
        return std::hash<uint64_t>{}(key.value);
    }
};
template <>
struct hash<realm_legacy::ColKey> {
    size_t operator()(realm_legacy::ColKey key) const
    {
        return std::hash<int64_t>{}(key.value);
    }
};
template <>
struct hash<realm_legacy::TableKey> {
    size_t operator()(realm_legacy::TableKey key) const
    {
