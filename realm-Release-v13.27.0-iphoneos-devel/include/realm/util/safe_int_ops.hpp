...
{
    return realm_legacy::_impl::SafeIntBinops<A, B>::equal(a, b);
}
...
{
    return realm_legacy::_impl::SafeIntBinops<A, B>::less(a, b);
}

template <class A, class B>
inline bool int_less_than_or_equal(A a, B b) noexcept
{
    return !realm_legacy::_impl::SafeIntBinops<B, A>::less(b, a); // Not greater than
}

template <class A, class B>
inline bool int_greater_than(A a, B b) noexcept
{
    return realm_legacy::_impl::SafeIntBinops<B, A>::less(b, a);
}

template <class A, class B>
inline bool int_greater_than_or_equal(A a, B b) noexcept
{
    return !realm_legacy::_impl::SafeIntBinops<A, B>::less(a, b); // Not less than
}
