...
public:
                                            ...
                                            util::Optional<ExpressionComparisonType> right_cmp_type)
    {
        ...
        if (left_size > 2 && right_size > 2) {
            ...
            else if constexpr (realm_legacy::is_any_v<TCond, Greater, GreaterEqual, Less, LessEqual>) {
        ...
        if constexpr (realm_legacy::is_any_v<TCond, BeginsWith, BeginsWithIns, EndsWith, EndsWithIns, Contains, ContainsIns,
                                      Like, LikeIns>) {
...
Query create(L left, const Subexpr2<R>& right)
{
    ...
    constexpr const bool supported_by_old_query_engine =
        ...
        (std::is_same_v<L, R> &&
         realm_legacy::is_any_v<L, double, float, Timestamp, StringData, BinaryData, ObjectId, UUID>);
...
public:
    ...
    std::vector<ObjKey> find_all(Mixed value) const final
    {
        ...
        if (m_link_map.get_target_table()->get_primary_key_column() == m_column_key) {
            ...
            REALM_ASSERT(index);
            if (value.is_null()) {
                index->find_all(result, realm_legacy::null{});
            }
            else {
                T val = value.get<T>();
        ...
        if (links_exist()) {
            ...
            if (m_link_map.only_unary_links()) {
                ...
                if (link_translation_key) {
                    ...
                    if constexpr (realm_legacy::is_any_v<T, ObjectId, UUID>) {
...
class Columns<BinaryData> : public SimpleQuerySupport<BinaryData> {
    ...
    friend Query operator==(const Columns<BinaryData>& left, realm_legacy::null)
    {
        return create<Equal>(BinaryData(), left);
    }

    friend Query operator==(realm_legacy::null, const Columns<BinaryData>& right)
    {
...
inline std::enable_if_t<!realm_legacy::is_any_v<T, StringData, realm_legacy::null, const char*, std::string>, Query>
...
public:
    ...
    void evaluate(size_t index, ValueBase& destination) override
    {
        if constexpr (realm_legacy::is_any_v<T, ObjectId, Int, Bool, UUID>) {
    ...
    {
        if constexpr (realm_legacy::is_any_v<T, Int, Float, Double, Decimal128, Mixed>) {
            return max().clone();
        }
        else {
            return {};
    ...
    {
        if constexpr (realm_legacy::is_any_v<T, StringData, BinaryData, Mixed>) {
...
class Columns : public ObjPropertyExpr<T> {
    constexpr static bool requires_null_column = realm_legacy::is_any_v<T, int64_t, bool>;
...
public:
    ...
    std::unique_ptr<Subexpr> max_of() override
    {
        if constexpr (realm_legacy::is_any_v<T, Int, Float, Double, Decimal128, Timestamp>) {
            return max().clone();
        }
        else {
            return {};
    ...
    std::unique_ptr<Subexpr> sum_of() override
    {
        if constexpr (realm_legacy::is_any_v<T, Int, Float, Double, Decimal128>) {
            return sum().clone();
        }
        else {
            return {};
