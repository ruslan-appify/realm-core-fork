...
{
    if (m_is_read_only)
        throw realm_legacy::LogicError(ErrorCodes::WrongTransactionState,
...
inline char* Allocator::translate_critical(RefTranslation* ref_translation_ptr, ref_type ref) const noexcept
{
...
#if REALM_ENABLE_ENCRYPTION
            realm_legacy::util::encryption_read_barrier(addr, NodeHeader::header_size, txl.encrypted_mapping,
    ...
    realm_legacy::util::terminate("Invalid ref translation entry", __FILE__, __LINE__, txl.cookie, 0x1234567890, ref, idx);
    return nullptr;
}
