...
public:
    using Iterator = realm_legacy::Dictionary::Iterator;
...
private:
    ...
    realm_legacy::Dictionary& dict() const noexcept
    {
        REALM_ASSERT_DEBUG(dynamic_cast<realm_legacy::Dictionary*>(m_coll_base.get()));
        return static_cast<realm_legacy::Dictionary&>(*m_coll_base);
    }
...
T Dictionary::get(StringData key) const
{
    ...
    if (res.is_null()) {
        if constexpr (std::is_same_v<T, Decimal128>) {
            return Decimal128{realm_legacy::null()};
        }
        else {
            return T{};
        }
