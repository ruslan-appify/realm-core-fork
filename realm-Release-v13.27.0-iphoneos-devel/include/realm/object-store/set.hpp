...
auto& Set::as() const
{
    REALM_ASSERT_DEBUG(dynamic_cast<realm_legacy::Set<T>*>(m_coll_base.get()));
    return static_cast<realm_legacy::Set<T>&>(*m_coll_base);
}
...
namespace std {
template <>
struct hash<realm_legacy::object_store::Set> {
    size_t operator()(realm_legacy::object_store::Set const&) const;
};
} // namespace std
