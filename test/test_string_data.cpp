...
TEST(StringData_Null)
{
    ...
    {
        StringData a = realm_legacy::null{};
        StringData b{""};
        CHECK(a < b);
        CHECK(b > a);
    }
...
{
    StringData null = realm_legacy::null();
    StringData empty("");
    StringData f("f");
    StringData foo("foo");
...
TEST(StringData_Hash_Murmur2)
{
    ...
    const uint_least32_t hash = realm_legacy::murmur2_32(reinterpret_cast<const unsigned char*>(input.data()), input.size());
...
TEST(StringData_Hash_CityHash64)
{
    ...
    for (size_t i = 0; i < num_checks; ++i) {
        ...
        const uint_least64_t hash = realm_legacy::cityhash_64(data, input_expected[i].first.size());
